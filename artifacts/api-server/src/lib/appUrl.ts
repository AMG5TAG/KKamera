 ...
 * var only if the app is ever hosted somewhere other than kkamera.app.
 */
export function getPublicBaseUrl(): string {
  const override = process.env["APP_URL"]?.trim();
  return (override && override.length > 0 ? override : "https://kkamera.app").replace(/\/+$/, "");
}

/** Host portion of the public base URL (e.g. "kkamera.app"). */
